    ...
    module VotingMethods
      module Borda
        describe ResponsesParser do
          ...
          describe "#parse" do
            context "with a valid contiguous ballot" do
              let(:payload) { { opt_a.id.to_s => 1, opt_b.id.to_s => 2, opt_c.id.to_s => 3 } }

              it "returns a ParsedResponses with responses and positions" do
                ballot = parser.parse(payload)

                expect(ballot).to be_a(Decidim::ExtraCensuses::VotingMethods::ParsedResponses)
                expect(ballot.responses).to contain_exactly(opt_a, opt_b, opt_c)
                expect(ballot.positions).to eq(opt_a.id => 1, opt_b.id => 2, opt_c.id => 3)
              end
            end
            ...
            context "when ranks are blank" do
              ...
              it "skips blank ranks" do
                ...
                expect(ballot.positions).to eq(opt_a.id => 1)
                expect(ballot.responses).to contain_exactly(opt_a)
              end
            end
          end

          describe "#validate!" do
            def build_ballot(positions)
            ...
            context "with a valid contiguous ballot" do
              it "does not raise" do
                ballot = build_ballot(opt_a.id => 1, opt_b.id => 2)
                expect { parser.validate!(ballot) }.not_to raise_error
              end
            end

            context "when the ballot is empty" do
              it "raises" do
                ballot = build_ballot({})
                expect { parser.validate!(ballot) }.to raise_error(StandardError)
              end
            end

            context "when the ballot is below min_choices" do
              it "raises" do
                ballot = build_ballot(opt_a.id => 1)
                expect { parser.validate!(ballot) }.to raise_error(StandardError)
              end
            end

            context "when the ballot is above max_choices" do
              let(:question) { create(:election_question, :borda, :voting_enabled, election:, max_choices: 2, min_choices: 1) }

              it "raises" do
                ballot = build_ballot(opt_a.id => 1, opt_b.id => 2, opt_c.id => 3)
                expect { parser.validate!(ballot) }.to raise_error(StandardError)
              end
            end
            ...
            context "when positions are not contiguous" do
              it "raises" do
                ballot = build_ballot(opt_a.id => 1, opt_b.id => 3)
                expect { parser.validate!(ballot) }.to raise_error(StandardError)
              end
            end
          end
